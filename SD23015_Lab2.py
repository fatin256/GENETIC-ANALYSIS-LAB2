...
def make_target_onemax(dim: int, target_ones: int, max_fit: float) -> GAProblem:
    """
    Fitness function for the custom requirement:
    1. Maximum fitness (80.0) is reached when num_ones == 50.
    2. Fitness decreases by the absolute difference from the target.
    """
    def fitness(x: np.ndarray) -> float:
        num_ones = np.sum(x)
        # Fitness = max_fit - absolute difference from target
        return float(max_fit - np.abs(num_ones - target_ones))

    return GAProblem(
        name=f"Target OneMax ({dim} bits, Target={target_ones})",
        chromosome_type="bit",
        dim=dim,
        bounds=None,
        fitness_fn=fitness,
...
def run_ga(
    ...
    _store_final(pop, fit) # Store final pop/fit
...
with st.sidebar:
    st.header("Problem")
    problem_type = st.selectbox("Type", ["Target OneMax (bits)", "OneMax (bits)", "Sphere (real)", "Rastrigin (real)"], index=0)

    if problem_type == "Target OneMax (bits)":
        dim = st.number_input("Chromosome length (bits)", min_value=8, max_value=4096, value=CHROMOSOME_LENGTH, step=8)
        target_ones = st.number_input("Target Number of Ones", min_value=1, max_value=int(dim), value=TARGET_ONES)
        max_fit = st.number_input("Max Fitness Value (at Target)", min_value=1.0, value=MAX_FITNESS)
        problem = make_target_onemax(int(dim), int(target_ones), float(max_fit))
    ...
    # Updated defaults for custom problem
    pop_size = st.number_input("Population size", min_value=10, max_value=5000, value=DEFAULT_POP_SIZE, step=10)
    generations = st.number_input("Generations", min_value=1, max_value=10000, value=DEFAULT_GENERATIONS, step=10)
...
with left:
    if st.button("Run GA", type="primary"):
        st.session_state["_final_pop"] = None # Clear previous results
        st.session_state["_final_fit"] = None # Clear previous results
        
        # Streamlit placeholder for temporary messages
        status = st.empty()
        ...
        if problem.chromosome_type == "bit":
            ...
            num_ones = int(np.sum(result['best']))
            st.write(f"Number of ones: **{num_ones}** / {problem.dim}")
            
            if problem.name.startswith("Target OneMax"):
                 st.write(f"Deviation from Target ({TARGET_ONES}): **{np.abs(num_ones - TARGET_ONES)}**")
        else:
...
# Store final population on each run for optional display (Initial state)
if "_final_pop" not in st.session_state:
    st.session_state["_final_pop"] = None
    st.session_state["_final_fit"] = None

# Function to store final results after run
